...
import org.meiconjun.erp4m.util.ZipUtil;
 ...
 */
public class WuneriguiDailyMail implements Job {
    private static Logger logger = LoggerFactory.getLogger(WuneriguiDailyMail.class);
    private static String mailFrom = "baoshiki@vip.qq.com";// 指明邮件的发件人
    private static String password_mailFrom = "nvusjflotbopbbaf";// 指明邮件的发件人登陆密码
//    private static String mailTo = "baoshiki@vip.qq.com"; // 指明邮件的收件人
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        logger.info("---------------------[job:无内鬼每日邮件推送]执行开始------------------------");
        //https://blog.csdn.net/qq_36474549/article/details/83342615
        // 打包当日文件 超过45M时分片
        curDate = DateUtil.format(new Date(), "yyyyMMdd");
        ...
        if (todayFile.exists()) {
            ...
            try {
                ...
                logger.info("邮件发送成功，删除昨日[" + yesterday + "]文件");
                // 删除昨日文件
                File yesTerDayFolder = new File(basePath + yesterday);
                FileUtil.del(yesTerDayFolder);
                File filess = new File(basePath);
                String[] namess = filess.list();
                for (String file : namess) {
                    if (file.startsWith(yesterday + ".z")) {
                        FileUtil.del(new File(basePath + file));
                    }
                }
     ...
     */
    public static MimeMessage createAttachMail(Session session, String title, String fileName) throws Exception {
