 ...
 * @Package
 * @Description:
 * @date 2020/8/22 22:29
 */
@Controller
public class BugListController extends BaseController {

    @Resource(name = "bugListServiceImpl")
    private BugListService bugListService;

    @ResponseBody
    @RequestMapping(value = "/buglist.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    public String excute(HttpServletRequest request, HttpServletResponse response) {
        return super.excuteRequest(request, new TypeToken<RequestBean<BugBean>>(){}.getType());
    }
    ...
    protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean) {
        ...
        try {
            responseBean = bugListService.excute(requestBean);
        } catch (Exception e) {
