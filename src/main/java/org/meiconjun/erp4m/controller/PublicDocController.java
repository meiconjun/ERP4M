...
public class PublicDocController extends BaseController {
    ...
    @Resource(name = "publicDocService")
    private PublicDocService publicDocService;

    @ResponseBody
    @RequestMapping(value = "/publicDoc.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    ...
    protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean) {
        ...
        try {
            responseBean = publicDocService.excute(requestBean);
        } catch (Exception e) {
